...
# 5 cores used for add_cluster (first loop)
# 1 core used for add_cluster (second_loop)

# 2 cores used for update_fragments
...
def neighbors_reductions(
    ...
    for i in outliers:
        ...
        all_bad_check = row if axis == 'row' else col
...
if __name__ == '__main__':
    missins = {
        "D01270" : ([], [9, 10, 11]), # (rows, columns)
        "D01355" : ([36], []), # (rows, columns)
        "D01332" : ([], [30,31,32,33,34]), # (rows, columns)
    }

    metrics_output = {}
    run_id = "D01332"
    metrics_output['run_id'] = run_id
    position_path = '/Users/joshuab/Desktop/ATX_impute/tests/D01332/tissue_positions_list_clusters.csv'
    fragments_path = '/Users/joshuab/Desktop/ATX_impute/tests/D01332/D01332_NG02803_fragments.tsv.gz'
    deviations = int(1)
    missing_rows = missins[run_id][0]
    missing_cols = missins[run_id][1]
    missing_lanes['row'] = sorted(missing_rows)
    missing_lanes['col'] = sorted(missing_cols)
    degree = 1
